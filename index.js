...
app.post('/api/supplier/products', checkUserAuth, upload.array('images', 6), async (req, res) => {
  if (req.user.role !== 'supplier') {
    return res.status(403).json({ success: false, error: 'هذه الميزة للموردين فقط' });
  }

  const { name, description, oem_number, category, condition, stock_quantity } = req.body;

  if (!name || !condition || stock_quantity === undefined) {
  ...
  try {
    ...
    const productResult = await pool.query(
      `INSERT INTO products (supplier_id, name, description, oem_number, category, condition, stock_quantity, is_active, approval_status, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, true, 'pending', NOW(), NOW())
       RETURNING id`,
      [supplierId, name, description || null, oem_number || null, category || null, condition, parseInt(stock_quantity)]
    );
    ...
    for (let i = 0; i < req.files.length; i++) {
      ...
      if (uploadError) {
        return res.status(500).json({ success: false, error: 'فشل رفع صورة: ' + uploadError.message });
      }
      ...
      await pool.query(
        ...
        [productId, publicUrlData.publicUrl, i]
      );
    }

    res.json({ success: true, message: 'تم إضافة المنتج، بانتظار موافقة الإدارة', product_id: productId });

  } catch (err) {
    res.status(500).json({ success: false, error: 'فشل إضافة المنتج: ' + err.message });
  }
});

app.post('/api/supplier/products/:id/pricing', checkUserAuth, async (req, res) => {
  if (req.user.role !== 'supplier') {
    return res.status(403).json({ success: false, error: 'هذه الميزة للموردين فقط' });
  }

  const { id } = req.params;
  const { vehicle_id, price } = req.body;

  if (!vehicle_id || !price) {
    return res.status(400).json({ success: false, error: 'المركبة والسعر مطلوبان' });
  }
  ...
  try {
    ...
    if (productCheck.rows.length === 0) {
      return res.status(404).json({ success: false, error: 'المنتج غير موجود أو ليس ملكك' });
    }

    await pool.query(
      `INSERT INTO product_vehicle_pricing (product_id, vehicle_id, price, is_active, approval_status, created_at, updated_at)
       VALUES ($1, $2, $3, true, 'pending', NOW(), NOW())`,
      [id, vehicle_id, price]
    );

    res.json({ success: true, message: 'تم إضافة السعر، بانتظار موافقة الإدارة' });

  } catch (err) {
    res.status(500).json({ success: false, error: 'فشل إضافة السعر: ' + err.message });
  }
});

app.get('/api/supplier/products', checkUserAuth, async (req, res) => {
  if (req.user.role !== 'supplier') {
    return res.status(403).json({ success: false, error: 'هذه الميزة للموردين فقط' });
  }
  ...
  try {
    ...
    const productsResult = await pool.query(
      `SELECT id, name, description, oem_number, category, condition, stock_quantity, approval_status, created_at
    ...
    for (const product of products) {
      ...
      const pricingResult = await pool.query(
        `SELECT pvp.id, pvp.vehicle_id, pvp.price, pvp.approval_status, 
      ...
      product.pricing = pricingResult.rows;
    }
...
// ============ إعداد أول حساب أدمن - محذوف بعد الاستخدام ============
...
app.get('/api/admin/pending-products', checkAdminAuth, requirePermission('can_manage_products'), async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT p.id, p.name, p.description, p.oem_number, p.category, p.condition, p.stock_quantity, p.created_at,
...
app.get('/api/admin/pending-pricing', checkAdminAuth, requirePermission('can_manage_products'), async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT pvp.id, pvp.product_id, pvp.price, pvp.created_at,
